...
#define COMMAND_TOGGLE_HELP_TEXT                              \
    " : Command to toggle LED" ENDLINE                        \
...
const Ifx_Shell_Command g_shellCommands[] = {
    ...
    {COMMAND_TOGGLE,
     COMMAND_TOGGLE_HELP_TEXT,
     &g_shellInterface,
     &toggleLEDShell},
    {COMMAND_HELP,
     COMMAND_HELP_HELP_TEXT,
     &g_shellInterface,
     &Ifx_Shell_showHelp},
...
void printInfo(IfxStdIf_DPipe *io)
{
    ...
    IfxStdIf_DPipe_print(io,
                         ...
                         "*************************" ENDLINE);
    IfxStdIf_DPipe_print(io,
                         "This is an example that shows how to use the "
                         "Infineon Shell from iLLDs.       " ENDLINE);
    IfxStdIf_DPipe_print(
        io,
        "In order to toggle the LED enter the command '" COMMAND_TOGGLE
        "' followed by one of the " ENDLINE);
    IfxStdIf_DPipe_print(io,
    ...
    IfxStdIf_DPipe_print(io,
                         ...
                         "                         " ENDLINE);
    IfxStdIf_DPipe_print(io,
                         "Any other parameter turns off the LED and reports a "
                         "Shell command error.      " ENDLINE);
    IfxStdIf_DPipe_print(io,
                         "*****************************************************"
                         "*************************" ENDLINE);
}
...
boolean toggleLEDShell(pchar args, void *data, IfxStdIf_DPipe *io)
{
    ...
    else
    {
        ...
        IfxStdIf_DPipe_print(io,
                             "Command syntax not correct." ENDLINE
                             "The correct syntax for this command is" ENDLINE
                             "    '" COMMAND_TOGGLE " [1]'" ENDLINE
...
void initSerialInterface(void)
{
    ...
    IfxAsclin_Asc_initModuleConfig(
        &ascConf,
        &MODULE_ASCLIN3); /* Initialize the structure with default values */

    /* Set the desired baud rate */
    ascConf.baudrate.baudrate = ASC_BAUDRATE; /* Set the baud rate in bit/s */
    ascConf.baudrate.oversampling =
        IfxAsclin_OversamplingFactor_16; /* Set the oversampling factor      */

    /* Configure the sampling mode */
    ascConf.bitTiming.medianFilter =
        IfxAsclin_SamplesPerBit_three; /* Set the number of samples per bit*/
    ascConf.bitTiming.samplePointPosition =
        IfxAsclin_SamplePointPosition_8; /* Set the first sample position    */

    /* ISR priorities and interrupt target */
    ascConf.interrupt.txPriority =
        ISR_PRIORITY_ASCLIN_TX; /* Set the interrupt priority for TX events */
    ascConf.interrupt.rxPriority =
        ISR_PRIORITY_ASCLIN_RX; /* Set the interrupt priority for RX events */
    ascConf.interrupt.erPriority =
        ISR_PRIORITY_ASCLIN_ER; /* Set the interrupt priority for Error events
                                 */
    ...
    const IfxAsclin_Asc_Pins pins = {
        ...
        .rx = &IfxAsclin3_RXD_P32_2_IN, /* Select the pin for RX connected to
                                           the USB port      */
        .rxMode  = IfxPort_InputMode_pullUp, /* RX pin */
        .rts     = NULL_PTR, /* RTS pin not used                 */
        .rtsMode = IfxPort_OutputMode_pushPull,
        .tx = &IfxAsclin3_TX_P15_7_OUT, /* Select the pin for TX connected to
                                           the USB port      */
    ...
    ascConf.txBufferSize =
        ASC_TX_BUFFER_SIZE; /* Set the transmission buffer size            */
    ascConf.rxBuffer = g_uartRxBuffer; /* Set the receiving buffer */
    ascConf.rxBufferSize =
        ASC_RX_BUFFER_SIZE; /* Set the receiving buffer size */

    /* Init ASCLIN module */
    IfxAsclin_Asc_initModule(
        &g_asclin,
        &ascConf); /* Initialize the module with the given configuration   */
}
...
void initShellInterface(void)
{
    ...
    //    Ifx_Console_print(ENDLINE "Enter '" COMMAND_HELP "' to see the
    //    available commands" ENDLINE);
    //
    //    /* Initialize the shell */
    //    Ifx_Shell_Config shellConf;
    //    Ifx_Shell_initConfig(&shellConf);                       /* Initialize
    //    the structure with default values         */
    //
    //    shellConf.standardIo = &g_ascStandardInterface;         /* Set a
    //    pointer to the standard interface              */
    //    shellConf.commandList[0] = &g_shellCommands[0];         /* Set the
    //    supported command list                       */
    //
    //    Ifx_Shell_init(&g_shellInterface, &shellConf);          /* Initialize
    //    the Shell with the given configuration    */
}
